...
def approve_leave(leave_id: str):
    ...
    if teacher_id is None or from_date_raw is None or to_date_raw is None:
        raise HTTPException(status_code=400, detail="Leave request record missing required fields.")
    ...
    try:
        ...
        try:
            if status_field:
                _update_leave_status(supabase, id_field=id_field, id_value=leave_id, status_field=status_field, new_status="pending")
        except Exception:
            pass
        traceback.print_exc()
